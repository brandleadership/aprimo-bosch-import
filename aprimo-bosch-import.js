...
createMeta = async (assetID, data, ImgToken) => {  
  ...
  for (var key in data) {
    ...
    let optionTrueFalse = "False";
    let ObjectID = '';
    let optionListValue = data[key]; //remove line break in option list
    ...
    if (typeof optionListValue === 'string') {
      optionListValue =  optionListValue.replace(/\n/g, '');
    }
    ...
    switch (key) {
      case 'AGENCY':// Option List
        ...
        if (ObjectID.hasOwnProperty('0')) {
          ...
          if (APIResult !== 'null') {
            ...
            dataFlag = false;
            dataFlagValue = ': DATA ERROR : Meta Key: ' + key + ' Value: ' + optionListValue;
            logger.error(new Date() + logRowInfo  + ': DATA ERROR : Meta Key: ' + key + ' Value: ' + optionListValue);
          }
        }else{
          dataFlag = false;
      ...
      case 'BRAND':// Option List
        ...
        if (ObjectID.hasOwnProperty('0')) {
          APIResult = await getfielddefinitionID(ObjectID[0]['_links']['definition']['href'], optionListValue, key)
          if (APIResult !== 'null') {
            updateObj.fields.addOrUpdate.push({
              "id": ObjectID[0].id,
              "localizedValues": [{
      ...
      case 'OBJ_ID':// Text
        ...
        break;
      case 'ORIGINAL_FILENAME': // Skip Block
        break;
      case 'OTYPE_ID':
        ...
        break;
      case 'OTYPE_NAME':// Skip Block
        break;
      case 'SYSTEM_STATUS'://classification
        ...
        break;
      case 'BINARY_FILENAME':// Skip Block
        break;
      case 'CSORELEASE_MASTER':
        ...
        if (ObjectID.hasOwnProperty('0')) {
          APIResult = await getfielddefinitionID(ObjectID[0]['_links']['definition']['href'], optionTrueFalse,  key)
          if (APIResult !== 'null') {
            updateObj.fields.addOrUpdate.push({
              "id": ObjectID[0].id,
              "localizedValues": [{
      ...
      case 'INIT_DATE':
        ...
        break;
      case 'DOUBLE_WIDTH':// Skip Block
        break;
      case 'HD_OBJECT':// Skip Block
        break;
      case 'MISSING_TTNR':// Text
        ...
        break;
      case 'AC_MAIN_USAGE':// Skip Block
        break;
      case 'LINKED_PRODUCTS':// Text
        ...
        break;
      case 'CLIPLISTER_LINKS':// Skip Block
        break;
      case 'COLOR_SPACE':// Text
      ...
      case 'PRIMEPOOL_BU':
        ...
        break;
      case 'RESPONSIBLE_BUSINESS_UNIT': //Classification
 ...
 * @param {*} filename 
 */
async function connectFtpWithRetry(config, retries, remotePath, filename) {
  let sftpRetry = new Client();
  await sftpRetry.connect(config)
  .then(async () => {
  ...
  .catch(async (err) => {
    ...
    if (retries > 0) {
      ...
      return new Promise(resolve => setTimeout(resolve, 4000))
        .then(() => connectFtpWithRetry(config, nextRetry, remotePath, filename));
    } else {
...
async function uploadAsset(filename, processPath) {
  ...
  await connectFtpWithRetry(ftpConfig, 5, remotePath, filename)
    .catch((err) => {
      dataFlag = false;
  ...
  if (fs.existsSync(filename) && BINARY_FILENAME !== '') {
    // Check Config for Azcopy Command
    if (APR_CREDENTIALS.azcopy) {
